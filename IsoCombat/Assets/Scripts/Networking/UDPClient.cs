...
public class UDPClient : INetwork
{
    ...
    public void Send(string text)
    {
        foreach (var line in text.Split('\n'))
        {
            ...
            string outLine = line.StartsWith("SYSTEM:")
                ? line
                : $"CHAT:{LocalName}:{line}";

            SendRaw(outLine + "\n");

            if (outLine.StartsWith("CHAT:"))
            {
                var p = outLine.Split(':');
                if (p.Length >= 3) OnChatMessage?.Invoke($"{p[1]}: {outLine.Substring(5 + p[1].Length)}");
            }
        }
    }
    ...
    {
        try { _sock.SendTo(System.Text.Encoding.UTF8.GetBytes(s), _server); }
